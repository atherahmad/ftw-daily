...
import css from './Slider_team.css';
import ReactDOM from 'react-dom';
import ExpandMoreIcon from '@material-ui/icons/ExpandMore';

import EmailIcon from '../../assets/icons/passion_petrol.png';
...
class Slide extends React.Component {
  ...
  render() {
    const { src, name, subHeadline, email, index } = this.props.slide;
    ...
    if (isMobile === false) {
      if (current === index) classNames = `${css.slide} ${css.slide__current}`;
      if (current + 1 === index) classNames = `${css.slide} ${css.slide__current}`;
      if (current + 2 === index) classNames = `${css.slide} ${css.slide__current}`;
      else if (current - 1 === index) classNames += `${css.slide} ${css.slide__previous}`;
      else if (current + 3 === index) classNames += `${css.slide} ${css.slide__next}`;
    } else {
      if (current === index) classNames = `${css.slide} ${css.slide__current}`;
      else if (current - 1 === index) classNames += `${css.slide} ${css.slide__previous}`;
    ...
    return (
      ...
      >
        <div className={css.slide__image__wrapper}>
          <img className={css.slide__image} alt={name} src={src} onLoad={this.imageLoaded} />
          <div className={css.gradient}></div>
          <article className={css.slide__content}>
            <h2 className={css.slide__headline}>{name}</h2>

            <h3 className={css.slide__subHeadline}>{subHeadline}</h3>

            {/* <p className={css.slide__email}>
              <div className={css.iconWrapper}>
                <img src={EmailIcon} width="35px" style={{ marginRight: '10px' }}></img>
              </div>
              {email}
            </p> */}
          </article>
...
class Slider_Team extends React.Component {
  constructor(props) {
    super(props);
